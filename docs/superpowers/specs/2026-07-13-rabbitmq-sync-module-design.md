...
RabbitMQ 模块通过 RabbitMQ Management HTTP API，在两个 RabbitMQ 集群之间同步同名 Virtual Host 下缺失的拓扑资源。同步遵循“只补充、不覆盖、不删除”的原则，不迁移消息，不修改源端，也不调整目标端已有资源。
...
源端客户端只允许发出读取请求。目标端客户端允许读取和创建请求。两个端点必须使用独立客户端实例，禁止共享可变的地址或认证状态。
...
RabbitMQ 模块不得提供删除或覆盖入口，也不得向任何 RabbitMQ 端点发送 `DELETE` 请求。
...
- `403 Forbidden`：提示账号缺少读取或创建权限，并展示对应资源类型。
...
主窗口顶部增加明确的 Nacos / RabbitMQ 模块切换控件。Nacos 模块保持现有布局，RabbitMQ 模块使用相同主题体系但拥有独立工作区。
...
- RabbitMQ 保存源端和目标端地址、用户名、加密密码、请求超时及最近选择的 Virtual Host。
...
- 所有目标写操作必须由同步计划驱动，界面层不能直接调用创建 API。
- 任何会引入覆盖、删除、消息迁移或跨名称 Virtual Host 映射的需求都属于后续独立设计范围。
